...
def respond(message, user_id):
    ...
    system_prompt = f"""
            ...
            1) Если пользователь хочет узнать назавния товаров используй функцию с именем show_product_names.
            ...
            Подстваляй chat_id - '{user_id}' в функции, где он нужен.
            Не придумывай ответы сам, используй функции.
            Не выдумывай ответы, если у тебя нет данных. 
            Если каких-то данных не хватает для вызова функции, то нужно спросить их у пользователя.
            Не вызывай функции, если у тебя недостаточно парамтеров для вызова.
            Не отвечай пользователю одно и то же. 
        """
